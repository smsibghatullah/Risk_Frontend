...
// checkbox functionalty start
 ...
 const series = [
  ...
  let get_programs_data = () => {
    ...
    axios.get(API_URL + `/audit_program/audit_engagement/${params.engagement_id}`)
      .then((res) => {
        const filteredData = res.data.filter(item => item.audit_engagement.toString() === params.engagement_id);
        setEngagementPrograms(filteredData.map(i => {
          try {
...
async function company_data(){
  ...
  useEffect(()=>{
    getdata(); get_programs_data();getdepartment();
    company_data();
        setTimeout(() => {
      ...
      <br/><br/><br/><br/><br/><br/>
         ...
         <center>
           <span className="mb-3">
            ...
            <span className="mb-3">
            <h1 style={{ marginLeft: '30%', marginRight: '30%', color: '#2f602f', fontFamily: 'Trebuchet MS', fontSize: '40px', fontWeight: 'bold' }}>
  {department.map((item) => {
    if (item.id === Engagement.department) {
      return item.name;
    }
    return null;
  })}
  <span className="" style={{ float: "right" }}>
    {/* Additional content */}
         ...
         <div style={{pageBreakAfter: 'always', margin: '0.5cm', breakAfter: 'page'}}></div>
        ...
        <td style={{ borderCollapse: 'collapse', border: "1px solid black", padding: "10px", fontFamily: 'Trebuchet MS', fontSize: '15px' }}>{eng.control_issue}</td>
           ...
           <tbody style={{borderCollapse:'collapse', border: "1px solid black"}}>
                <tr style={{borderCollapse:'collapse', border: "1px solid black",backgroundColor:'#3c4b64',color:'white'}}> 
                  ...
                  {
    pro.our_observation && pro.our_observation.map((obs) => (
